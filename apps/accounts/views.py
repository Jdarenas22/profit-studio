...
def trainer_dashboard(request):
    ...
    context = {
        'total_clients': User.objects.filter(role='member').count(),
        ...
        'total_routines': Routine.objects.filter(is_active=True).count(),
        'recent_clients': User.objects.filter(role='member').order_by('-date_joined')[:6],
        'recent_assessments': InitialAssessment.objects.select_related('user').order_by('-date')[:5],
    }
